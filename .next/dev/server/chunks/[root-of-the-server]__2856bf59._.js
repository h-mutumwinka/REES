...
"[project]/Music/RuralEducation/app/api/admin/dashboard/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const user = database.prepare("SELECT name FROM users WHERE id = ? AND role = ?").get(userId, "admin");
        if (!user) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$Music$2f$RuralEducation$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        const totalStudents = database.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'student'").get().count;
        const totalTeachers = database.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'teacher'").get().count;
        const totalCourses = database.prepare("SELECT COUNT(*) as count FROM courses").get().count;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__2856bf59._.js.map